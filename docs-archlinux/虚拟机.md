...
```
sudo docker system prune -a --volumes && sudo systemctl disable --now docker.service && yay -Rns winboat-bin && sudo rm -rfv /var/lib/docker /etc/docker ~/.docker /var/run/docker ~/.winboat ~/.config/winboat && reboot 
```
