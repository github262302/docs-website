...
- [niri使用clipse剪贴板程序](#niri使用clipse剪贴板程序)
...
- [vim/nvim退出插入模式自动切换为英文输入](#vimnvim退出插入模式自动切换为英文输入)
...
1. 安装
    ...
    `wl-clipboard`提供`wl-copy`和`wl-paste`工具

    `clipse-bin`是已经编译好的clipse，-bin是binary代表二进制，如果-git就是源码编译，如果没有后缀就是最新的发行版本
...
5. 浮动模式打开clipse-gui
    ...
    点选clipse-gui的窗口获取窗口信息，主要使用title或者app-id

    然后编辑niri的配置文件搜索`window-rule`，在合适的位置自己写一个窗口规则的代码块。

    或者搜`open-floating`，niri的默认配置里有一个window-rule代码块是这个规则。

    ```
    window-rule {
        //match app-id用appid指定窗口规则生效的窗口，也可以使用title
...
6. 禁用选取后自动粘贴
    ...
    clipse-gui的配置文件里`enter_to_paste = False`可以禁用复制即粘贴，但是似乎没有生效。解决办法是把`paste_simulation_cmd_wayland=`里的命令改成true。
...
PS：clipse-gui可能因为python更新暂时无法使用。可以使用clipse提供的tui。终端运行clipse即可打开。为了方便使用可以设置快捷键自动打开kitty运行clipse。

```
Mod+Alt+V {spawn-sh "kitty --class clipse -e clipse";}
```

`--class`指定这个终端的app-id为clipse，目的是方便设置浮动窗口规则。
