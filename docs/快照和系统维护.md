...
5. 设置合理的快照策略
   ...
   - 修改设置
      ...
      其他的`TIME_LIMIT`数量都设置为0。这样就仅保存三个小时前的状态和昨天的状态。
...
- btrfs-assistant命令行（推荐）
   ...
   2. 用snapper序号找到对应的**btrfs-assistant序号**
      ...
      有趣的是，这条命令会把home和root的快照排在同一个列表里。图中snapper序号为`11`的home快照对应着btrfs-assistant序号`6`（这意味着btrfs-assistant序号1~5是root快照，6往后是home快照）。
...
- snapper命令行
   ...
   arch的子卷布局不支持`snapper rollback`命令，只能使用`undochange`命令回档。
...
- 从快照启动项进入系统后`snapper list`没能列出快照
...
- 滚挂
  ...
  Arch Linux是滚动发行版。滚动是英文直译，原词是rolling，指一种推送更新的方式，只要有新版本就会推送，由用户管理更新。对应的另一种更新方式是定期更新一个大版本，例如fedora是六个月一更新，由发行方管理更新。 滚挂，指的是滚动更新的发行版因为更新导致系统异常。不用担心，只要学习一下正确的更新方式和快照的使用方法就不用担心滚挂问题。
...
<details><summary>我觉得这很好玩，你可以跳过这部分内容。</summary>
...
5. 回档
   ...
   这条命令把`@_bak1/.snapshots`目录下序号为`1`的快照变成了新的`@`。
